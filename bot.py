...
def extract_content_id(url):
    ...
    if "contentid=" not in lower_url:
        return None

    # contentid= ka exact index lower_url se nikalo
    start = lower_url.find("contentid=") + len("contentid=")

    # original url se value nikalo taaki original case preserve rahe
    content_id = url[start:].strip()
    ...
    if content_id.lower().endswith(".m3u8"):
        content_id = content_id[:-5]                                         
    # remove trailing slash
    content_id = content_id.rstrip("/")

    # if plus became space accidentally
    content_id = content_id.replace(" ", "+")                                
    return content_id


def get_signed_url(url, raw_text4):
    content_id = extract_content_id(url)

    if not content_id:
        print("Invalid Link / contentId not found")
        return None
    ...
    api_url = (
        ...
        f"?contentId={encoded_content_id}"
    )
    ...
    try:
        ...
        if isinstance(response, dict):
            ...
            elif response.get("drmUrls"):
                final_url = response["drmUrls"].get("manifestUrl")
...
@bot.on_message(filters.private & filters.command(["ankit","deaduser"]) )
...
def extract_content_id(url):
    ...
    content_id = url.split("contentId=", 1)[1].strip()
...
@bot.on_message(filters.private & filters.command(["member","misthi"]))
async def txt_handler(bot: Client, m: Message):
    user_id = m.from_user.id
    if not is_auth_user(user_id):
        await m.reply_text("**HEY BUDDY THIS IS ONLY FOR MY ADMINS**")
        ...
        for i in range(arg-1, len(links)):
            ...
            if "visionias" in url:
                ...
                if not content_id:
                    print("Invalid Link / contentId not found")
                    return None
                encoded_content = urllib.parse.quote(content_id, safe="")
                
                # FIXED: Proper indentation and using aiohttp
                headers = {
                    'host': 'api.classplusapp.com',
                    'x-access-token': f'{raw_text4}',    
                    'accept-language': 'EN',
...
def convert_pw_link(url):
    """
    Convert PW dash links to master.m3u8
    Handles: dash/audio/2.mp4, dash/audio/22.mp4, dash/240/2.mp4, etc.
    """
    import urllib.parse
    
    # First decode if it's URL encoded
    decoded = urllib.parse.unquote(url)
    
    # Use regex to replace ANY dash/* pattern with master.m3u8
